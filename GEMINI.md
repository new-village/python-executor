...
## 基本方針
- **汎用実行基盤**: 1つのコンテナイメージに全てのタスク（スクリプト）を詰め込み、実行時にコンテナ引数（ARGS）で動的に処理（モジュール名）を切り替える「全部入り」構成を維持する。
- **Cloud Run Jobs 特化**: バッチ実行に最適化し、標準出力による Cloud Logging 連携を活用する。独自ディスパッチャは持たず、`ENTRYPOINT ["python", "-m"]` による最小構成を保つ。
